...
def main():
    ...
    term_to_linked_terms.update(
        linked_terms(efo_cellline_og, cvcl_og, link_syn_types=["EXACT", "RELATED"])
    )
    term_to_linked_terms.update(
        linked_terms(cvcl_og, efo_cellline_og, link_syn_types=["EXACT", "RELATED"])
    )

    with open("term_to_linked_terms.json", "w") as f:
        f.write(
            json.dumps(
                term_to_linked_terms, indent=4, sort_keys=True, separators=(",", ": ")
            )
        )
...
def linked_terms(og_a, og_b, link_syn_types=None):
    ...
    for b_term in list(og_b.id_to_term.values()):
        ...
        b_ref_strs += [
            syn.syn_str for syn in b_term.synonyms if syn.syn_type in link_syn_types
        ]
