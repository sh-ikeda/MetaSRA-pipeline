...
def main():
    ...
    (
        sample_to_predicted_terms_all,
        sample_to_real_val_props_all,
    ) = get_all_samples_to_mappings(mapping_f)
    ...
    for sample_acc, tag_to_values in sample_to_tag_to_values.items():
        ...
        try:
            n_grams = lc.get_ngrams_from_tag_to_val(sample_to_tag_to_values[sample_acc])
        except:
            print("Error retrieving n-grams!")
            n_grams = []
        feat_v = vectorizer.convert_to_features(
            n_grams, sample_to_predicted_terms_all[sample_acc]
        )
    ...
    with open(out_f, "w") as f:
        json.dump(sample_to_prediction, f, indent=True, sort_keys=True)
