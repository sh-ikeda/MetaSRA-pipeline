...
def mutual_info_rank_features(feature_vecs, binary_labels):
    ...
    ranked_indices = [
        index
        for (mi, index) in sorted(zip(mutual_infos, [x for x in range(num_features)]))
    ]
...
class OneVsRestClassifier:
    ...
    def fit(self, feature_vecs, labels):
        ...
        for clss in classes:
            ...
            self.filt_features.update(
                mutual_info_rank_features(feature_vecs, train_labels)[
                    self.feature_cutoff :
                ]
            )
    ...
    def predict(self, q_feature_v, predicted_terms, real_value_props):
        ...
        if self.use_predicted_term_rules:
            ...
            if not is_xenograft:
                for real_val_prop in real_value_props:
                    if (
                        real_val_prop["property_id"] == "EFO:0007061"
                    ...
                    if pred_term.split(":")[0] == "CVCL":
                        for subset in self.cvcl_og.id_to_term[pred_term].subsets:
                            if subset in cellosaurus_subset_to_possible_types:
                                ...
                                # ) # TODO REMOVE
                                zero_types = all_types.difference(
                                    set(cellosaurus_subset_to_possible_types[subset])
                                )
        ...
        if VERBOSE:
            sys.stderr.write(
                "Class to confidence before normalizing: %s" % class_to_confidence
            )
            sys.stderr.write("Sum before normalizing: %f" % sum_conf)
        if sum_conf > 0:
            class_to_confidence = {
                k: v / sum_conf for k, v in class_to_confidence.items()
            }
        if VERBOSE:
            sys.stderr.write("Class to confidence: %s" % class_to_confidence)
        return max([(k, v) for k, v in class_to_confidence.items()], key=lambda x: x[1])
