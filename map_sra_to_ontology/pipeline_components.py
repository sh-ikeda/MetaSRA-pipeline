...
FILTER_KEYS_JSON = pr.resource_filename(
    resource_package, join("metadata", "filter_key_val_rules.json")
)
CELL_LINE_FILTER_KEYS_JSON = pr.resource_filename(
    resource_package, join("metadata", "cell_line_filter_key_val_rules.json")
)
PROPERTY_SPECIFIC_SYNONYMS_JSON = pr.resource_filename(
    resource_package, join("metadata", "has_val_syn_term_ids.json")
)
TERM_TO_LINKED_ANCESTOR_JSON = pr.resource_filename(
    resource_package, join("metadata", "term_to_superterm_linked_terms.json")
)
NOUN_PHRASES_JSON = pr.resource_filename(
    resource_package, join("metadata", "noun_phrases.json")
)
CELL_LINE_TO_IMPLIED_DISEASE_JSON = pr.resource_filename(
    resource_package, join("metadata", "cellline_to_disease_implied_terms.json")
)
ACRONYM_TO_EXPANSION_JSON = pr.resource_filename(
    resource_package, join("metadata", "acronym_to_expansions.json")
)
REAL_VALUE_PROPERTIES = pr.resource_filename(
    resource_package, join("metadata", "real_valued_properties.json")
)
CUST_TERM_TO_CONSEQ_TERMS_JSON = pr.resource_filename(
    resource_package, join("metadata", "custom_term_to_consequent_terms.json")
)
CELL_LINE_TERMS_JSON = pr.resource_filename(
    resource_package, join("metadata", "cvcl_mappings.json")
)
TWO_CHAR_MAPPINGS_JSON = pr.resource_filename(
    resource_package, join("metadata", "twochar_or_numeric_cellline.json")
)
TERM_ARTIFACT_COMBOS_JSON = pr.resource_filename(
    resource_package, join("metadata", "term_artifact_combo.json")
)
PRIOR_KEY_TO_ONT_JSON = pr.resource_filename(
    resource_package, join("metadata", "prioritizing_key_to_ont.json")
)
# PRIOR_KEY_TO_ONT_JSON = pr.resource_filename(resource_package, join("metadata", "prioritizing_key_to_ont_plant.json"))
CVCL_TO_CELLLINE_JSON = pr.resource_filename(
    resource_package, join("metadata", "cvcl_term_to_other_cellline_terms.json")
)
AMBIGUOUS_KEYS_JSON = pr.resource_filename(
...
class RealValueProperty:
    def __init__(
        self, property_id, consequent, value, unit_id, orig_key, orig_val, mapping_path
    ):
...
# def unwrap_pipeline_run(arg, **kwarg):
#     return Pipeline.run(*arg, **kwarg)
    ...
    def extract_mapped_terms(self, text_mining_graph):
        ...
        def extract_mapping(mapped_node):
            ...
            dist, prev = text_mining_graph.shortest_path(
                mapped_node, use_reverse_edges=True
            )
                ...
                while c_node != mapped_node:
                    ...
                    if isinstance(prev[c_node][0], TokenNode):
                        if origin_gram_pos == (-1, -1):
                            origin_gram_pos = (
                                prev[c_node][0].origin_gram_start,
                                prev[c_node][0].origin_gram_end,
                            )
                            used_gram_pos = (
                                prev[c_node][0].origin_gram_start,
                                prev[c_node][0].origin_gram_end,
        ...
        mapped_terms = []
        exclude_ids = set(
            [x.property_term_id for x in text_mining_graph.real_value_nodes]
        )
        exclude_nodes = set(
            [
                x
...
class KeyValueFilter_Stage:
    ...
    def run(self, text_mining_graph):
        ...
        if self.perform_filter_keys:
            remove_kv_nodes = [
                x for x in text_mining_graph.key_val_nodes if x.key in self.filter_keys
            ]
...
class CellosaurusSynonyms_Stage(Synonyms_Stage):
    def __init__(self):
        super(CellosaurusSynonyms_Stage, self).__init__(
            "Cellosaurus", "cvcl_syn_sets.json"
        )


class ManuallyAnnotatedSynonyms_Stage(Synonyms_Stage):
    def __init__(self):
        super(ManuallyAnnotatedSynonyms_Stage, self).__init__(
            "Manually Annotated", "custom_syn_sets.json"
        )
                        ...
                        for t_node in text_mining_graph.forward_edges[kv_node][edge]:
                            for down_node in text_mining_graph.downstream_nodes(t_node):
                                if isinstance(down_node, TokenNode):
                                    for syn_set in self.property_id_to_syn_sets[
                                        key_term_node.term_id
                                    ]:
    ...
    def run(self, text_mining_graph):
        ...
        for kv_node in text_mining_graph.key_val_nodes:
            ...
            key_term_nodes = [
                ...
                for x in key_term_nodes
                if (
                    isinstance(x, OntologyTermNode) and x.term_id in self.cell_line_keys
                )
                or (
                        ...
                        for down_node in text_mining_graph.downstream_nodes(t_node):
                            if isinstance(down_node, OntologyTermNode):
                                if down_node.term_id in self.cell_line_terms:
                                    ...
                                    # cell line term node
                                    dist, prev = text_mining_graph.shortest_path(
                                        down_node, use_reverse_edges=True
                                    )
                                    ...
                                    if not path_from_cell_line_key:
                                        ...
                                        for edge in text_mining_graph.reverse_edges[
                                            down_node
                                        ]:
                                            for (
                                                token_node
                                            ) in text_mining_graph.reverse_edges[
                                                down_node
                                            ][
                                                edge
                                            ]:
                                                if (
                                                    token_node
                                                    in text_mining_graph.downstream_nodes(
                                                        t_node
                                                    )
                                                ):
                                                    remove_edges.append(
                                                        (token_node, down_node, edge)
                                                    )
    ...
    def run(self, text_mining_graph):
        ...
        for t_node_1 in text_mining_graph.token_nodes:
            ...
            for t_node_2 in text_mining_graph.token_nodes:
                if (
                    t_node_2.origin_gram_start == start
                    and t_node_2.origin_gram_end == end
                ):
        ...
        for t_node_set in matched_t_node_sets:
            ...
            if found_exact:
                for t_node in t_node_set:
                    for edge, target_nodes in text_mining_graph.forward_edges[
                        t_node
                    ].items():
                        if isinstance(edge, FuzzyStringMatch) and edge.edit_dist > 0:
                            #                            print "Found match w/ edit distance > 0: %s --%s--> %s" % (t_node, edge, target_nodes)
                            remove_edge_args += [
                                (t_node, target_node, edge)
                                for target_node in target_nodes
                            ]
...
class Delimit_Stage:
    ...
    def __init__(self, delimiter, replace_to_space=True):
        self.delimiter = delimiter
        self.replace_to_space = replace_to_space
...
class FilterOntologyMatchesByPriority_Stage:
    ...
    def run(self, text_mining_graph):
        ...
        def is_edge_to_node_a_match(edge, targ_node, id_space):
            return (
                isinstance(edge, FuzzyStringMatch)
        ...
        id_spaces = set(
            [x.term_id.split(":")[0] for x in text_mining_graph.ontology_term_nodes]
        )
            ...
            for t_node in text_mining_graph.token_nodes:
                ...
                for edge, target_nodes in text_mining_graph.forward_edges[
                    ...
                    for targ_node in target_nodes:
                        if is_edge_to_node_a_match(
                            edge, targ_node, id_space
                        ) and is_edge_direct_match(edge):
                            discard = True
                            break
                    if discard:
                        break
                ...
                if discard:
                    ...
                    for edge, target_nodes in text_mining_graph.forward_edges[
                        ...
                        for targ_node in target_nodes:
                            if is_edge_to_node_a_match(
                                edge, targ_node, id_space
                            ) and not is_edge_direct_match(edge):
...
class FuzzyStringMatching_Stage:
    ...
    def __init__(
        ...
        fname = pr.resource_filename(
            resource_package, join("fuzzy_matching_index", index_json)
        )
        with open(fname, "r") as f:
            self.str_to_terms = json.load(f)

        fname = pr.resource_filename(
            resource_package, join("fuzzy_matching_index", index_pickle)
        )
        # with open(fname, "r") as f:
    ...
    def _edit_below_thresh(self, query):
        ...
        for result in within_edit_thresh:
            ...
            if VERBOSE:
                print(
                    "Retrieved '%s' from BK-tree. It has edit distance of %f"
                    % (str2.encode("utf-8"), dist)
                )
            len1 = len(str1)
            len2 = len(str2)
    ...
    def run(self, text_mining_graph, covered_query_map):
        ...
        for t_node in text_mining_graph.token_nodes:
            ...
            if VERBOSE:
                print(
                    "Searching %dth node in the BK-tree: '%s'" % (c, t_node.token_str)
                )
            ...
            for m in matched:
                ...
                text_mining_graph.add_edge(
                    ...
                    match_node,
                    FuzzyStringMatch(
                        t_node.token_str, matched_str, match_type, edit_dist=edit_dist
                    ),
                )
...
class TermArtifactCombinations_Stage:
    ...
    def run(self, text_mining_graph):
        all_artifacts = set(
            [a_node.token_str for a_node in text_mining_graph.token_nodes]
        )
        all_term_ids = set(
            [ont_node.term_id for ont_node in text_mining_graph.ontology_term_nodes]
        )
        ...
        for combo in self.term_artifact_combos:
            ...
            for ont_node in required_ont_nodes:
                add_edges.append((ont_node, OntologyTermNode(conseq_term_id), edge))
        for e in add_edges:
                    ...
                    for t_node in text_mining_graph.forward_edges[kv_node][edge]:
                        for down_node in text_mining_graph.downstream_nodes(t_node):
                            if isinstance(down_node, OntologyTermNode):
                                ...
                                if (
                                    down_node.namespace()
                                    == self.key_to_ont[kv_node.key]["ontology"]
                                ):
                                    tobreak = True
...
class RemoveSubIntervalOfMatchedBlockAncestralLink_Stage:
    def run(self, text_mining_graph):
        def is_superphrase(super_node, sub_node):
            matched_interval = (
                super_node.origin_gram_start,
                super_node.origin_gram_end,
            )
            if (
                sub_node.origin_gram_start == matched_interval[0]
                and sub_node.origin_gram_end < matched_interval[1]
            ):
        ...
        for t_node in mapped_t_nodes:
            ...
            if VERBOSE:
                print(
                    "\n\nNode %s has superphrase nodes: %s"
                    % (t_node, superphrase_nodes)
                )
            # ここから
            ...
            superphrase_node_to_reachable = {
                x: text_mining_graph.downstream_nodes(x, exclude_edges=exclude_edges)
                for x in superphrase_nodes
            }

            mapped_from_t = [
                x
                for x in text_mining_graph.get_children(t_node)
                if isinstance(x, MappingTargetNode)
            ]
            ...
            for mft in mapped_from_t:
                ...
                for (
                    superphrase_node,
                    reachable_from_superphrase,
                ) in superphrase_node_to_reachable.items():
                ...
                if reachable_from_all_supernodes:
                    if VERBOSE:
                        print(
                            "The node %s that is reachable from the token node is reachable from all of the superphrase nodes."
                            % mft
                        )
            ...
            for edge in text_mining_graph.forward_edges[t_node]:
                for targ_node in text_mining_graph.forward_edges[t_node][edge]:
                    reachable_from_targ_node = set(
                        text_mining_graph.downstream_nodes(targ_node)
                    )
                    if (
                        len(reachable_from_targ_node.intersection(keep_as_mappable))
                        == 0
                    ):
                        del_edges.append((t_node, targ_node, edge))
                    else:
                        if VERBOSE:
                            print(
                                "Target node from the current node, %s, can reach a node that we want to keep as mappable: %s"
                                % (
                                    targ_node,
                                    reachable_from_targ_node.intersection(
                                        keep_as_mappable
                                    ),
                                )
                            )
...
class ExtractRealValue_Stage:
    ...
    def run(self, text_mining_graph):
        ...
        for kv_node in text_mining_graph.key_val_nodes:
            if VERBOSE:
                print(
                    "Checking whether key-value pair node %s encodes a \
                    real-value property."
                    % kv_node
                )
            ...
            real_val_term_nodes = [
                ...
                if isinstance(x, OntologyTermNode) and x.term_id in self.real_val_tids
            ]
            if VERBOSE:
                print(
                    "Set of real-value properties we are searching \
                    for: %s"
                    % self.real_val_tids
                )
            ...
            numeric_nodes = [
                ...
                if isinstance(x, TokenNode) and is_number(x.token_str)
            ]
            if VERBOSE:
            ...
            unit_nodes = [
                x
                for x in real_val_candidates
                if isinstance(x, OntologyTermNode) and x.term_id.split(":")[0] == "UO"
            ]
            if VERBOSE:
            ...
            )  # TODO should use a different edge-type
            if len(real_val_term_nodes) == 1:
                ...
                for numeric_node in numeric_nodes:
                    if len(unit_nodes) == 1:
                        ...
                        rv_node = RealValuePropertyNode(
                            prop_term_node.term_id, float(numeric_node.token_str), None
                        )
                        text_mining_graph.add_edge(prop_term_node, rv_node, edge)
                        text_mining_graph.add_edge(numeric_node, rv_node, edge)
        return text_mining_graph
    ...
    def run(self, text_mining_graph):
        ...
        for kv_node in text_mining_graph.key_val_nodes:
            ...
            for edge in text_mining_graph.forward_edges[kv_node]:
                if isinstance(edge, DerivesInto) and edge.derivation_type == "key":
                    for t_node in text_mining_graph.forward_edges[kv_node][edge]:
                        key_time_nodes.update(
                            text_mining_graph.downstream_nodes(t_node)
                        )
            key_time_nodes = [
                x
                for x in key_time_nodes
                if isinstance(x, OntologyTermNode) and x.term_id in self.time_nodes
            ]
...
class ImpliedDevelopmentalStageFromAge_Stage:
    def run(self, text_mining_graph):
        for real_node in text_mining_graph.real_value_nodes:
            if (
                real_node.property_term_id == "EFO:0000246"
    ...
    def run(self, text_mining_graph):
        ...
        for o_node in text_mining_graph.ontology_term_nodes:
            if o_node.namespace() == "CVCL" and o_node.term_id in self.cvcl_to_mappings:
                ...
                for real_val in self.cvcl_to_mappings[o_node.term_id][
                    "real_value_properties"
                ]:
                    new_rv_node = RealValuePropertyNode(
                        real_val[0], real_val[1], real_val[2]
                    )
            ...
            for edge in text_mining_graph.forward_edges[kv_node]:
                if isinstance(edge, DerivesInto) and edge.derivation_type == "val":
                    for t_node in text_mining_graph.forward_edges[kv_node][edge]:
                        ...
                        if (
                            kv_node.key not in self.ambiguous_keys
                            and len(keyname_to_o_nodes[keyname]) != 0
                        ):
                            to_remove = True
...
def main():
    tag_to_val = {"cell type": "T cell", "marker": "CD4+"}
    ...
    exact_match = ExactStringMatching_Stage(
        ["1", "2", "4", "5", "7", "8", "9"], query_len_thresh=3
    )
    # fuzzy_match = FuzzyStringMatching_Stage(0.1, query_len_thresh=3)
    ...
    result = {
        "Mapped ontology terms": [x.to_dict() for x in mapped_terms],
        "Real valued properties": [x.to_dict() for x in real_val_props],
    }
